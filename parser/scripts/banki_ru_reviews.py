...
class BankiReviews:
            ...
            for i in range(1, page_num + 1):
                ...
                for bank in page.find_all("tr", {"data-test": "banks-list-item"}):
                    ...
                    if id_exist and license_id is not None:
                        cbr_bank = session.exec(
                            cbr_banks.where(Banks.id == license_id)
                        ).one()
                    else:
                        continue
                    ...
                    banks.append(
                        InfoBankiRu(
                            bank_name=bank_link.text,
                            reviews_url=bank_url,
                            bank=cbr_bank,
                        )
                    )
    ...
    def parse(self) -> None:
        with Session(engine) as session:
            ...
            parsed_time = (
                source.last_checked if source.last_checked is not None else datetime.min
            )
            browser = get_browser()
            for bank_index, bank in enumerate(bank_list):
                reviews_list = []
                self.logger.info(
                    f"[{bank_index+1}/{len(bank_list)}] Start parse bank {bank.bank_name}"
                )
                browser.get(bank.reviews_url)
                self.logger.debug(f"Send request to {bank.reviews_url}")
                page = BeautifulSoup(browser.page_source, "html.parser")

                reviews_num_elem_main = page.find(
                    "div", class_="ui-pagination__description"
                )
                ...
                for i in range(1, page_num + 1):
                    ...
                    for response in responses:
                        ...
                        rating = (
                            int(rating_text.text) if rating_text is not None else None
                        )
