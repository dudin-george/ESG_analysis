...
from banki_ru.database import BankiRuBank, BankiRuBroker, BankiRuMfo
...
class BankiMfo(BankiBase):
    ...
    url = f"https://www.banki.ru/microloans/responses/ajax/responses/"
    per_page = 200
    params = {"perPage": per_page, "grade": "all", "status": "all"}

    def get_mfo_json(self, page: int = 1) -> dict[str, Any]:
        params = {"catalog_name": "main", "period_unit": 4,
                  "region_ids[]": ["433", "432"],
                  "page": page,
                  "per_page": 48,
                  "total": 206,
    ...
    def get_mfo_json_reviews(self, bank: BankiRuBank, page: int = 1) -> dict[str, Any]:
    ...
    def load_bank_list(self) -> None:
        ...
        unique_mfos = list({(company["mfo"]["name"], int(company["mfo"]["ogrn"]), company["mfo"]["code"]) for company in microfin})
        return_microfin = []
        ...
        banks_db = [BankiRuMfo.from_pydantic(bank) for bank in return_microfin]
    ...
    def get_page_bank_reviews(self, bank: BankiRuBank, page_num: int, parsed_time: datetime) -> list[Text] | None:
        ...
        reviews = []
        for review in reviews_json["responses"]["data"]:
            text = Text(
                title=review["title"],
                text=review["text"],
