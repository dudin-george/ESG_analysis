...
from app.query.source import get_source_items, create_source, get_source_types_items
from app.shemes import GetSource, CreateSource, PostSourceResponse, GetSourceItem, GetSourceTypes, GetSourceTypesItem
...
async def post_source(source: CreateSource, db: Session = Depends(get_db)):
...
async def get_source_types(db: Session = Depends(get_db)):
    ...
    for source_item in source_types:
        get_source_item = GetSourceTypesItem(
            id=source_item.id,
